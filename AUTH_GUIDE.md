...
應用採用**手動同步模式**，只有在用戶點擊「立即同步到雲端」按鈕時才會同步。

```javascript
1. 用戶手動點擊"立即同步到雲端" (manualSync)
   → 先上傳本地數據到雲端
   → 再從雲端拉取最新數據
   → 合併本地和雲端數據
   → 顯示同步狀態和結果
...
**A:** 可以！每個設備都有完整的本地副本。請在每個設備上點擊「立即同步到雲端」按鈕來同步最新數據。
...
✅ **完全離線** - IndexedDB 本地存儲  
✅ **手動同步** - Supabase 雲端備份（點擊「立即同步到雲端」按鈕）  
✅ **安全隔離** - RLS 策略保護  
✅ **智能清理** - 365 天自動過期  
✅ **跨設備** - 多端數據同步  
